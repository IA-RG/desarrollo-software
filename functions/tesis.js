async function obtenerTesis(consulta, promisePool, res) {
  const [ids, fields] = await promisePool
    .query(consulta)
    .catch((err) => res.status(404).send("Error. Intente de nuevo"));
  // console.log(ids);

  return await obtenerArregloDeTesis(promisePool, res, ids);
}

async function obtenerArregloDeTesis(promisePool, res, ids) {
